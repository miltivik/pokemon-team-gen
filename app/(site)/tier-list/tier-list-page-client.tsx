...
interface TierListPageClientProps {
  initialTierData: Record<string, TierListEntry[]>;
}

export default function TierListPageClient({ initialTierData }: TierListPageClientProps) {
  const { t, lang } = useTranslation();
  const isSpanish = lang === "es";
  const hasInitialData = Object.keys(initialTierData).length > 0;
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      for (const tier of Object.keys(grouped)) {
        grouped[tier].sort((left, right) => right.usage - left.usage);
      }
  ...
  useEffect(() => {
    if (skipInitialFetch.current && format === "gen9ou") {
          ...
          {loading ? (
            ...
            TIER_ORDER.filter((tier) => tierData[tier]?.length > 0).map((tier) => (
              <div key={tier} className="space-y-3">
                <div className={`flex items-center gap-3 px-4 py-2 rounded-lg border ${TIER_COLORS[tier]}`}>
                  <h2 className="text-xl font-bold">{tier} Tier</h2>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-col min-w-0">
                          <span
                            className="font-semibold text-sm truncate dark:text-zinc-200"
                            title={mon.name}
                          >
                            {mon.name}
                          </span>
            ...
            <Card className="w-full max-w-2xl mx-auto bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
              <CardContent className="text-center py-12">
                <p className="text-zinc-600 dark:text-zinc-400 mb-4">
                  Live usage data is temporarily unavailable. You can retry without leaving this page.
                </p>
